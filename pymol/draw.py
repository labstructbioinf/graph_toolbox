...
class PyMolColor:
    ...
    def draw_all_cofactors(self, directory, chain, chain_name, pdb_list, embeddings, scale=False, **kw_args):
        '''
        ...
        '''
        if scale:
            embeddings = self.emb.scaler_with_conserved_sign(embeddings)
        ...
        for num_cof in range(4):
            cof_scores = embeddings[:, num_cof].ravel()
            self.init_script(path_preffix, name_cof, chain, num_cof, pdb_list, cof_scores)
        #copy pdb strucute to `directory`

        #create csv with values
        df = pd.DataFrame(embeddings, columns = list(LABEL_DICT_R.keys()))
        df['pdb_list'] = list(pdb_list)
        df.to_csv(path_preffix + '/residue_scores.csv')
